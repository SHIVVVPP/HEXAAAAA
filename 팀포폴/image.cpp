...
void image::frameRender(HDC hdc, int destX, int destY, int width, int height, int currentFrameX, int currentFrameY)
{
	_imageInfo->currentFrameX = currentFrameX;
	_imageInfo->currentFrameY = currentFrameY;
...
void image::aniRender(HDC hdc, int destX, int destY, int width, int height, animation * ani)
{
...
{
	alphaFrameRender(hdc, destX, destY, ani->getFramePos().x/ ani->getFrameWidth(), ani->getFramePos().y/ ani->getFrameHeight(), alpha);
}
...
void image::stretchRender(HDC hdc, int destX, int destY, float scale)
{
		TransparentBlt(
			hdc,
			destX, destY,
			_imageInfo->width*scale, _imageInfo->height*scale,
